...
Compatibility with ECH extension will be provided with the proxy must ignoring
unknown parameters, and generating its own ClientHello containing only
parameters it understands. 

Thus, when presenting a certificate to the client or sending a ClientHello
to the server, the proxy will act as if connecting to the ClientHelloOuter
